...
(function (global, factory) {
	...
	(global = global || self, global.Fsm = factory());
	...
	*/
	 ...
	 *
	 */
	var darkenRatio = 0.15;
	snap_svg.plugin(function(Snap, Element, Paper) {
		//  get all circles
		...
		Paper.prototype.label = function(x, y, options) {
			var color$1 = options.color; if ( color$1 === void 0 ) color$1 = "#fff";
			var position = options.position; if ( position === void 0 ) position = "left";
			var text = options.text; if ( text === void 0 ) text = "";
			var textSize = options.textSize; if ( textSize === void 0 ) textSize = 14;

			var arrowW = 18,
				labelWidth = 120,
				labelHeight = 30,
				tailWidth = 6;
			var labelPath = "M" + x + "," + y + " L" + (x - arrowW) + "," + (y - labelHeight / 2) + " L" + (x -
				labelWidth) + "," + (y - labelHeight / 2) + " L" + (x - labelWidth) + "," + (y +
				labelHeight / 2) + " L" + (x - arrowW) + "," + (y + labelHeight / 2) + " L" + x + "," + y;
			var tailPath = "M" + (x - labelWidth) + "," + (y - labelHeight / 2) + " L" + (x -
				labelWidth -
			...
			var shadowF = this.filter(Snap.filter.shadow(1, 2, 2, "#999", 0.5));
	...
	State.prototype.renderCircle = function renderCircle (cx, cy, r) {
		var circle = this._canvas.paper.circle(cx, cy, r, this.color);
		circle.r = r;
		circle.cx = cx;
		circle.cy = cy;
		this.g.add(circle);
	...
	State.prototype.renderLabel = function renderLabel (cx, cy, pos) {
		var labelOffset = 10,
	...
	*/
	...
	var ANIME_DURATION = 1600;
	var ANIME_EASING_BEZIER = "cubicBezier(.5, .05, .1, .3)";
	...
	Fsm.prototype.resetState = function resetState (index, options) {
			var this$1 = this;
			if ( options === void 0 ) options = {};

		assert(__initialized, "fsm.init must be called first.");

		var oldState = this._states[index];
		var ref = Object.assign({}, oldState, options);
			var circleR = ref.circleR;
		...
		// repaint links
		var tLinks = this.links.filter(function (link) {
			return link.t === index || link.s === index;
		});
		forEachValue(tLinks, function (tlink) {
			var tPath = tlink.path;
			var linkPoint = calcLinkPoint(this$1, tlink.s, tlink.t);
	...
	function link$1(fsm, sIndex, tIndex) {
		var stateS = fsm._states[sIndex];
		var stateT = fsm._states[tIndex];
		if (!stateS || !stateT) {
			return;
		}
		...
		var gradi = fsm.canvas.paper.gradient(
			("L(" + (p1.x) + ", " + (p1.y) + ", " + (p2.x) + ", " + (p2.y) + ")" + (stateS.g.circle.stroke) + "-" + (stateT.g.circle.stroke))
		);
	...
	function calcLinkPoint(fsm, sIndex, tIndex) {
		var stateS = fsm._states[sIndex];
		var stateT = fsm._states[tIndex];
		var theta1 = stateS.theta;
		var theta2 = stateT.theta;
		var tranformAngle = 360 / fsm._states.length;
		var curvQuot = tranformAngle / 90;
		var curv1 = -0.58 * curvQuot;
		var curv2 = 0.58 * curvQuot;
		var offsetRad = 18;
